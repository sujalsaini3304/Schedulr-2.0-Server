...
router.post("/api/createUser", async (req, res) => {
  ...
  try {
    if (username && password) {
      ...
      if (profileImageUrl) {
        await client.query(
          `INSERT INTO users (username , password , profile_image_url) VALUES ( '${username}' , '${key}' , '${profileImageUrl}')`
        );
      } else {
        await client.query(
...
router.post("/api/deleteUser", async (req, res) => {
  ...
  try {
    ...
    if (response) {
      ...
      if (flag) {
        await client.query(`DELETE FROM users WHERE username = '${username}'`);
        res.status(200).json({
...
router.post("/api/verifyUser", async (req, res) => {
  ...
  const { username, password } = req.body;
  try {
    const value = await becrypt.hash(password, 8);
    const response = await client.query(
...
router.post("/api/set/schedule", jwtAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      await client.query(
        `INSERT INTO schedule (username , day ,from_time ,to_time, period , subject , branch , section , teacher) VALUES ( '${username}' ,'${day}' , '${from_time}', '${to_time}' , '${period}' , '${subject}', '${branch}', '${section}', '${
...
router.post("/api/update/profileName", jwtAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    const response2 = await client.query(
      `UPDATE schedule SET teacher = '${new_name}' WHERE username = '${username}'`
    );
...
router.post("/api/update/details", jwtAuthMiddleware, async (req, res) => {
  ...
  try {
    if (new_branch && username) {
      await client
        .query(
          `UPDATE schedule SET branch = '${new_branch}' WHERE username = '${username}'`
        )
        .then((e) => {
          res.status(200).json({
    ...
    if (new_subject && username) {
      await client
        .query(
          `UPDATE schedule SET subject = '${new_subject}' WHERE username = '${username}'`
        )
        .then((e) => {
          res.status(200).json({
    ...
    if (new_section && username) {
      await client
        .query(
          `UPDATE schedule SET section = '${new_section}' WHERE username = '${username}'`
        )
        .then((e) => {
          res.status(200).json({
    ...
    if (new_from_time && username) {
      await client
        .query(
          `UPDATE schedule SET from_time = '${new_from_time}' WHERE username = '${username}'`
        )
        .then((e) => {
          res.status(200).json({
    ...
    if (new_to_time && username) {
      await client
        .query(
          `UPDATE schedule SET to_time = '${new_to_time}' WHERE username = '${username}'`
        )
        .then((e) => {
          res.status(200).json({
    ...
    if (new_period && username) {
      await client
        .query(
          `UPDATE schedule SET period = '${new_period}' WHERE username = '${username}'`
        )
        .then((e) => {
          res.status(200).json({
    ...
    if (new_day && username) {
      await client
        .query(
          `UPDATE schedule SET day = '${new_day}' WHERE username = '${username}'`
        )
        .then((e) => {
          res.status(200).json({
            message: "day changed",
